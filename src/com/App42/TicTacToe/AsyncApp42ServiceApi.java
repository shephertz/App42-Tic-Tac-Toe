...
public class AsyncApp42ServiceApi {
	...
	private AsyncApp42ServiceApi(){
    	...
    	//sp.setBaseURL("http://", "182.73.181.76", 8084);
    	this.userService = sp.buildUserService();
    	this.sessionService = sp.buildSessionManager();  
	...
	public void getUserGamesList(final String uname, final App42ServiceListener callBack){
		final Handler callerThreadHandler = new Handler();
        new Thread(){
            @Override public void run() {
                try {
                	String collName = Constants.App42UserGamesCollectionPrefix + uname;
                	Storage response = storageService.findAllDocuments(Constants.App42DBName, collName);
                	final ArrayList<JSONDocument> jsonDocList = response.getJsonDocList();
                	callerThreadHandler.post(new Runnable() {
                        @Override
                        public void run() {      
                        	callBack.onGetUserGamesList(Utilities.getJSONObjectsFromJSONDocuments(jsonDocList));
                        }            			
            		});            		
                }  
	...
	public void createGame(final String uname1, final String remoteUserName, final App42ServiceListener callBack)
	{
		final Handler callerThreadHandler = new Handler();
        new Thread(){
            @Override public void run() {
				try {
					// Validate remoteUser
					User remoteUser = userService.getUser(remoteUserName);
					// create object
					...
					gameObject.put(Constants.GameIdKey, java.util.UUID.randomUUID().toString());
					
					// Insert in to user1's game collection					
					storageService.insertJSONDocument(Constants.App42DBName, Constants.App42UserGamesCollectionPrefix + uname1, gameObject.toString());					
					// Insert in to user2's game collection
					storageService.insertJSONDocument(Constants.App42DBName, Constants.App42UserGamesCollectionPrefix + remoteUserName, gameObject.toString());
					callerThreadHandler.post(new Runnable() {
                        @Override
                        public void run() {      
	...
	public void updateGame(final JSONObject newGameObj, final App42ServiceListener callBack)
	{
		final Handler callerThreadHandler = new Handler();
		new Thread(){
			@Override public void run() {	
				...
				try {
					String collName1 = Constants.App42UserGamesCollectionPrefix + newGameObj.getString(Constants.GameFirstUserKey);
					String collName2 = Constants.App42UserGamesCollectionPrefix + newGameObj.getString(Constants.GameSecondUserKey);
					String id = newGameObj.getString(Constants.GameIdKey);
					storageService.updateDocumentByKeyValue(Constants.App42DBName, collName1, Constants.GameIdKey, id, newGameObj.toString());
					storageService.updateDocumentByKeyValue(Constants.App42DBName, collName2, Constants.GameIdKey, id, newGameObj.toString());
            		callerThreadHandler.post(new Runnable() {
                        @Override
                        public void run() {      
	...
	public void pushMessage(final JSONObject newGameObj, final String userName)
	{
		...
		new Thread(){
			@Override public void run() {	
				try {
					pushService.sendPushMessageToUser(Constants.GameName+userName, newGameObj.toString());
            		callerThreadHandler.post(new Runnable() {
                        @Override
                        public void run() {      
